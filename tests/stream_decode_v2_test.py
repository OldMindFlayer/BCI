...
class Decode():
    def __init__(self, config, inlet_amp, inlet_pn):
        ...
        self.fingersrange = np.array([40])
        #avatar channels for finger joints
        self.fsendrange = np.array([3])
    ...
    def decode(self):
        ...
        while (time.time() - start_time < self.exp_time):
            ...
            if chunk_size > 0:
                ...
                prediction = np.copy(self.pnbuff)
                ...
                self.avatarBuffer[self.fsendrange+6] = prediction
        
                self.avatarBuffer[self.fsendrange+48] = fact
                self.avatarBuffer[self.fsendrange+3+48] = fact
                self.avatarBuffer[self.fsendrange+6+48] = fact
